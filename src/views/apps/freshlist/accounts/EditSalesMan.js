...
const EditSalesMan = ({ EditOneData }) => {
  const [CreatAccountView, setCreatAccountView] = useState([]);
  const [Countries, setCountry] = useState({});
  const [States, setState] = useState({});
  const [Cities, setCities] = useState({});
  ...
  const submitHandler = (e) => {
    ...
    if (error) {
      ...
      UpdateSalespersonsave(EditOneData?._id, formData)
        .then((res) => {
          console.log(res);
          setFormData({});
          if (res.status) {
            // window.location.reload();
            swal("SalesMan Updated Successfully");
          }
        })
        .catch((err) => {
    ...
    <div>
      <div>
        <Card>
          <Row className="m-2">
            <Col>
              <h1 className="float-left">Edit Sales Man</h1>
            </Col>
            <Col>
              {/* <div className="float-right">
                <Route
