...
import {
  ...
  UpdateSalespersonsave,
  Update_Sales_Managersave,
} from "../../../../ApiEndPoint/ApiCalling";
import { BiEnvelope } from "react-icons/bi";
import { FcPhoneAndroid } from "react-icons/fc";
import { BsWhatsapp } from "react-icons/bs";
...
const EditSalesManager = ({ EditOneData }) => {
  const [CreatAccountView, setCreatAccountView] = useState([]);
  const [Countries, setCountry] = useState({});
  const [States, setState] = useState({});
  const [Cities, setCities] = useState({});
  ...
  useEffect(() => {
    ...
    CreateSalesManagerXMlView()
      .then((res) => {
        ...
        if (JSON.parse(jsonData)?.Createsalesmanager?.dropdown) {
          setdropdownValue(JSON.parse(jsonData)?.Createsalesmanager?.dropdown);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const submitHandler = (e) => {
    ...
    if (error) {
      ...
      Update_Sales_Managersave(EditOneData?._id, formData)
        .then((res) => {
          console.log(res);
          setFormData({});
          if (res.status) {
            // window.location.reload();
            swal("SalesMananger Details Updated Successfully");
          }
        })
        .catch((err) => {
    ...
    <div>
      <div>
        <Card>
          <Row className="m-2">
            <Col>
              <h1 className="float-left">Edit SalesManager </h1>
            </Col>
            <Col>
              {/* <div className="float-right">
                <Route
